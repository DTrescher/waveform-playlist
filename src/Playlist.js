...
export default class {
    ...
    setUpEventEmitter() {
        ...
        ee.on("solo", (track, state) => {
            if (typeof track == "number")
                this.soloTrackIdx(track, state);
            else
                this.soloTrack(track);
            this.adjustTrackPlayout();
            this.drawRequest();
        ...
        ee.on("mute", (track, state) => {
            if (typeof track == "number")
                this.muteTrackIdx(track, state);
            else
                this.muteTrack(track);
            this.adjustTrackPlayout();
            this.drawRequest();
    ...
    */
    ...
    muteTrackIdx(index, state) {
        const track = this.tracks[index];

        if (!track) {
            console.log("muteTrackIdx: invalid index -> track not found!");
            return;
        }

        const _index = this.mutedTracks.indexOf(track);
        ...
        switch (state) {
            ...
            case false:
                if (_index > -1) {
                    this.mutedTracks.splice(_index, 1);
                }
                return;
        }
    ...
    soloTrackIdx(index, state) {
        const track = this.tracks[index];

        if (!track) {
            console.log("soloTrackIdx: invalid index -> track not found!");
            return;
        }

        const _index = this.soloedTracks.indexOf(track);
        ...
        switch (state) {
            ...
            case false:
                if (_index > -1) {
                    this.soloedTracks.splice(_index, 1);
                }
                return;
        }
    ...
    updateEditor(cursor) {
        ...
        if (this.isPlaying()) {
            ...
            this.resetDrawTimer = setTimeout(() => {
                ...
                // following code fixes the cursor alignment after finish playing
                this.playbackSeconds = this.cursor ? this.cursor : 0; // this.playbackSeconds = 0;
                this.ee.emit("timeupdate", this.playbackSeconds);
                this.draw(this.render());
            }, 0);
        }
    }
